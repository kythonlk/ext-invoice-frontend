...
import { FileSignature, Lock, User, ShieldCheck, ArrowRight, CheckCircle2, Sparkles, Download, Building2 } from 'lucide-react';
import { triggerPwaInstall } from '../components/PwaInstallPrompt';

function Login({ onLogin }) {
  const [companies, setCompanies] = useState([]);
  const [selectedCompany, setSelectedCompany] = useState(localStorage.getItem('selectedCompanyCode') || '');
    ...
    const fetchCompanies = async () => {
      try {
        ...
        if (Array.isArray(res.data) && res.data.length > 0) {
          ...
          const matched = res.data.find(c => c.CompanyCode === saved);
          if (matched) {
            setSelectedCompany(matched.CompanyCode);
          } else {
        ...
        console.error("Failed to load ERP companies:", err);
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const compCode = res.data.user.CompanyCode || res.data.user.company_code || selectedCompany;
      localStorage.setItem('selectedCompanyCode', compCode);
      onLogin(res.data.user, res.data.token);
    } catch (err) {
      setError(err.response?.data?.error || 'Unable to sign in. Please check your Focus ERP credentials and company.');
    } finally {
      setLoading(false);
    }
  };

  const selectedCompObj = companies.find(c => c.CompanyCode === selectedCompany);
      ...
      <section className="hidden lg:flex relative overflow-hidden bg-[#15183b] text-white p-14 xl:p-20 flex-col justify-between">
        ...
        <div className="relative max-w-xl">
          ...
          <div className="mt-10 grid grid-cols-2 gap-4 max-w-lg">
            {['Multi-company isolation', 'Live ERP synchronization', 'Multi-level approval rules', 'Verified e-signatures'].map((item) => (
        ...
        <div className="relative flex items-center gap-3 text-xs text-indigo-200/65">
          <ShieldCheck className="w-4 h-4 text-emerald-300" /> Enterprise-grade multi-company ERP access control
        </div>
      </section>
      ...
      <section className="relative flex min-h-screen items-center justify-center overflow-hidden px-5 py-10 sm:px-10">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="relative w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="mt-2 text-sm text-slate-500">Select your ERP company and enter your credentials.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <label htmlFor="company" className="text-xs font-bold text-slate-700 flex items-center justify-between">
              ...
              <div className="relative">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    localStorage.setItem('selectedCompanyCode', e.target.value);
                  }}
                  disabled={loadingCompanies || loading}
                  required
                  className="w-full pl-10 pr-9 py-3.5 bg-slate-50/80 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 text-slate-900 text-sm font-semibold transition-all appearance-none cursor-pointer disabled:opacity-60"
                >
                  {loadingCompanies ? (
                    <option value="">Loading ERP companies...</option>
                  ) : companies.length > 0 ? (
                    companies.map((c) => (
                    ...
                    <option value="0D0">GOC (0D0 - Focus80D0)</option>
                  )}
                </select>
            ...
            <div className="space-y-2">
              ...
              <div className="relative"><User className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                <input
                  ...
                  required
                  className="w-full pl-10 pr-4 py-3.5 bg-slate-50/80 border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 text-slate-900 text-sm font-medium transition-all placeholder:text-slate-400"
                  placeholder="Enter username (e.g. su or 8040_su)"
                />
              </div>
            </div>
            <div className="space-y-2">
            ...
            <button
              ...
              className="group w-full min-h-12 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-xl shadow-indigo-200/70 transition-all text-sm flex items-center justify-center gap-2 disabled:opacity-60"
            >
              {loading ? (
                <><span className="w-4 h-4 rounded-full border-2 border-white/40 border-t-white animate-spin" />Authenticating…</>
              ) : (
                <>Continue to workspace <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" /></>
              )}
            </button>
          </form>
